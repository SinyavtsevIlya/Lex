...
namespace Nanory.Lex
{
    ...
    [Il2CppSetOption (Option.ArrayBoundsChecks, false)]
        ...
        public sealed class Mask
        {
            ...
            internal int Hash;
            internal List<EcsLocalFilterContainer> LocalCache;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public Mask With<T>() where T : struct
            {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public Mask Without<T>() where T : struct
            {
                var poolId = _world.GetPool<T>().GetId();
#if DEBUG
                if (_built) { throw new Exception("Cant change built mask."); }
            ...
            {
                var hasLocalCache = LocalCache != null;
                var localHash = 0;
#if DEBUG
                if (_built) { throw new Exception("Can't change built mask."); }
                _built = true;
#endif
                if (hasLocalCache)
                {
                    // local hash can be unsorted
                    localHash = CalculateHash();

                    for (int i = 0; i < LocalCache.Count; i++)
                    {
                        if (LocalCache[i].Hash == localHash)
                        {
                            var localFilter = LocalCache[i].Filter;
                            Recycle();
                            return localFilter;
                        }
                    }
                ...
                if (hasLocalCache)
                {
                    LocalCache.Add(new EcsLocalFilterContainer(filter, localHash));
                }
                if (!isNew) { Recycle(); }
                return filter;
            }
            ...
            int CalculateHash()
            {
                var hash = IncludeCount + ExcludeCount;
            ...
            internal static Mask New(EcsWorld world, List<EcsLocalFilterContainer> localCache)
            {
                var mask = New(world);
                mask.LocalCache = localCache;
                return mask;
            }
        }
    ...
    public class EcsLocalFilterContainer
    {
        public EcsFilter Filter;
        public int Hash;

        public EcsLocalFilterContainer(EcsFilter filter, int hash)
        {
            Filter = filter;
            Hash = hash;
        }
